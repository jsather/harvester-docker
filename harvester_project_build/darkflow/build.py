...
class TFNet(object):
            ...
            with tf.device(device_name):
                if new_sess: 
                    with self.graph.as_default() as g:
                        self.build_from_pb(new_sess=new_sess)
                else: 
                    self.build_from_pb(new_sess=new_sess)
            return
    ...
    def build_from_pb(self, new_sess = True):
        ...
        self.last_hidden = tf.get_default_graph().get_tensor_by_name(
            '21-leaky:0')
        self.smallest_hidden = tf.get_default_graph().get_tensor_by_name(
            '14-maxpool:0')
        
        self.setup_meta_ops(new_sess=new_sess)
    ...
    def setup_meta_ops(self, new_sess=True):
        cfg = dict({
